...
csv_dir = os.path.join(cache_dir, 'CSVs')

# Lista os arquivos GeoJSON na pasta cache
geojson_files = [f for f in os.listdir(cache_dir) if f.endswith('.geojson')]

for geojson_file in geojson_files:
    year = geojson_file.split('_')[1].split('.')[0]  # Extrai o ano do nome do arquivo

    with open(os.path.join(cache_dir, geojson_file), 'r') as geojson_file:
    ...
    with open(os.path.join(csv_dir, f"Data_{year}.csv"), 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerows(csv_data)

    df = pd.read_csv(os.path.join(csv_dir, f'Data_{year}.csv'))
    ...
    df.to_csv(os.path.join(csv_dir, f'Data_{year}_Resume.csv'), index=False)